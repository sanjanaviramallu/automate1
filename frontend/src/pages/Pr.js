...
function Pr() {
  ...
  const handleSubmit = (e) => {
    ...
    doc.text(
      36,
      60,
      `Seeking permission to promote public relations activities regarding "${formData.name}" in the college.`
    );
    doc.text(
      20,
      70,
    ...
    doc.text(
      20,
      80,
      `proposed to conduct an ${formData.event}. As a part of this event we request you to grant permission for`
    
    );
    doc.text(20, 90, `conducting PR in college on ${format}.`);
  ...
  return (
    <div className="bg-slate-100 min-h-screen flex items-center justify-center bg-gray-100 overflow-hidden pb-12 ">
      ...
      <div className="mx-auto flex w-full max-w-sm flex-col gap-6 bg-slate-300 box-border h-35 w-50  h-90 p-4 flex items-center border-4 rounded-xl mt-20 flex flex-col items-center">
        <h2 className="text-3xl font-semibold">Pr Permission</h2>
