...
function Audi() {
  ...
  const handleSubmit = (e) => {
    ...
    doc.text(
      37,
      60,
      `Request to book auditorium on ${formData.date} for the event "${formData.Event}".`
    );
    ...
    doc.text(
      20,
      80,
      `conducting an event ${formData.Event} on ${formData.date} from ${formData.FromTime} to ${formData.ToTime}. We request you to book auditorium on ${formData.date}.
      `
    );
  ...
  <input
    ...
    onChange={handleChange}
    className="input w-50 h-10"
    min="09:00" // Sets the minimum time value to 9:00 AM
  />
</div>
